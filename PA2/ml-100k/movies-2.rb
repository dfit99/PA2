...
class MovieTest
	...
	#returns rms by looping through and using typical formula
        def rms 
                diff =0
                count = 0.0
                @data.each do |line|
                        ...
                        guess_rating = line[3]
                        count =  count + 1
                        diff  = (true_rating.to_i - guess_rating.to_i)**2 + diff
                end

                diff = diff/count
                diff_avg = Math.sqrt(diff)
                return diff_avg.round(2)
        end

	#returns stddev
        def stddev
		variance = @data.inject(0) do |sum , line|
			true_rating = line[2]
                        guess_rating = line[3]
			sum +  ((true_rating.to_i - guess_rating).abs - mean)**2
		end
